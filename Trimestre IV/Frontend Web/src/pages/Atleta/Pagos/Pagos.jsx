...
// ID fijo de prueba (coincide con el tipo string usado en los datos: "1", "2", "4"...)
const ATLETA_ID_PRUEBA = "5";

function PagosAtleta() {
  const atletaId = ATLETA_ID_PRUEBA;
  ...
  useEffect(() => {
    let vigente = true;
    Promise.all([
      api.obtener("atletas", atletaId),
      api.listar(`mensualidades?atletaId=${atletaId}`),
    ])
      .then(([a, m]) => {
        if (vigente) {
          setAtleta(a);
